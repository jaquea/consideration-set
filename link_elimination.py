...
from k_shortest_paths import link_elimination_observed_parameters
...
viajes_link_elimination_shortest_alternativas_desaglosadas_procesados_observed_parameters = defaultdict(lambda: defaultdict(list))

viajes_link_elimination_shortest_alternativas_procesados_observed_parameters = defaultdict(lambda: defaultdict(list))
...
for origen in viajes_alternativas_procesados:
    for destino in viajes_alternativas_procesados[origen]:
        ...
        k_paths_link_elimination = link_elimination(g, origin_index, destination_index, num_k, weights, dict_servicio_llave_codigoTS, paradero_cercano_dic)
...
#Construccion de alternativas con parametros de modelos con datos observados
...
for origen in viajes_alternativas_procesados:
    for destino in viajes_alternativas_procesados[origen]:
        ...
        start_time = time.clock()
        k_paths_link_elimination = link_elimination_observed_parameters(g, origin_index, destination_index, num_k, weights, dict_servicio_llave_codigoTS, paradero_cercano_dic)
        print "link_elimination", time.clock() - start_time, "seconds"
        ...
        for path in caminos_resumidos:
            if path not in viajes_link_elimination_shortest_alternativas_procesados_observed_parameters[origen][destino]:
                costo_path = costos[contador]
                camino = caminos_desglosados[contador]

                viajes_link_elimination_shortest_alternativas_desaglosadas_procesados_observed_parameters[origen][destino].append(camino)
                viajes_link_elimination_shortest_alternativas_procesados_observed_parameters[origen][destino].append(path)
            contador += 1

dump_file2 = open('tmp\\viajes_link_elimination_shortest_alternativas_desaglosadas_procesados_observed_parameters.pkl', 'wb')
dill.dump(viajes_link_elimination_shortest_alternativas_desaglosadas_procesados_observed_parameters, dump_file2)
dump_file2.close()

dump_file2 = open('tmp\\viajes_link_elimination_shortest_alternativas_procesados_observed_parameters.pkl', 'wb')
dill.dump(viajes_link_elimination_shortest_alternativas_procesados_observed_parameters, dump_file2)
dump_file2.close()
